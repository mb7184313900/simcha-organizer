...
// Single source of truth for "which side is the current user on."
// weddingRow.side_a_role ('chosson' | 'kallah') is the one shared fact for
// the whole wedding -- Side A's own answer. Side B's side is always the
// computed opposite, never stored independently. Returns null if side_a_role
// hasn't been set yet.
export function getMySide(weddingRow, isSideB) {
  const sideARole = weddingRow?.side_a_role
  if (!sideARole) return null
  if (!isSideB) return sideARole
  return sideARole === 'chosson' ? 'kallah' : 'chosson'
}
